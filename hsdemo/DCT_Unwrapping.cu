...
void myDCT(float *ImgDst, float *ImgSrc, int sizeX, int sizeY)
{
	...
	//DCT on x-axis
	float2cufft << <grid, block >> >(dSrc_tmp, ImgSrc, sizeX, sizeY);
	//cuFFT1D(dSrc_tmp, sizeX * 2, sizeY, -1);	
	cufftExecC2C(cuFFT1D_plan1_FORWARD, dSrc_tmp, dSrc_tmp, CUFFT_FORWARD);

	countFDCT << <grid, block >> >(ImgDst, dSrc_tmp, sizeX, sizeY, 1 / sqrtf(2 * sizeY));
	transposeShared << <dimGrid, dimBlock >> >(ImgSrc, ImgDst);

	//DCT on y-axis	
	float2cufft << <grid, block >> >(dSrc_tmp, ImgSrc, sizeX, sizeY);
	//cuFFT1D(dSrc_tmp, sizeY * 2, sizeX, -1);
	cufftExecC2C(cuFFT1D_plan2_FORWARD, dSrc_tmp, dSrc_tmp, CUFFT_FORWARD);

	countFDCT << <grid, block >> >(ImgSrc, dSrc_tmp, sizeY, sizeX, 1 / sqrtf(2 * sizeY));
...
void myIDCT(float *ImgDst, float *ImgSrc, int sizeX, int sizeY)
{
	...
	//IDCT on x-axis
	transposeShared << <dimGrid, dimBlock >> >(ImgDst, ImgSrc);	
	countIDCT << <grid, block >> >(dSrc_tmp, ImgDst, sizeX, sizeY, 1 / sqrtf(2 * sizeY));
	//cuFFT1D(dSrc_tmp, sizeX * 2, sizeY, 2);	
	cufftExecC2C(cuFFT1D_plan1_INVERSE, dSrc_tmp, dSrc_tmp, CUFFT_INVERSE);
	cufft2float << <grid, block >> >(ImgSrc, dSrc_tmp, sizeX, sizeY);
	
	//IDCT on y-axis
	transposeShared << <dimGrid, dimBlock >> >(ImgDst, ImgSrc);
	countIDCT << <grid, block >> >(dSrc_tmp, ImgDst, sizeY, sizeX, 1 / sqrtf(2 * sizeY));
	//cuFFT1D(dSrc_tmp, sizeY * 2, sizeX, 2);	
	cufftExecC2C(cuFFT1D_plan2_INVERSE, dSrc_tmp, dSrc_tmp, CUFFT_INVERSE);
	cufft2float << <grid, block >> >(ImgDst, dSrc_tmp, sizeX, sizeY);
}
//--------------------------------------------------------------------------------------
...
__global__ void countFDCT(float *ImgDst, cufftComplex *ImgSrc, int sizeX, int sizeY, float dTemp)
{
	...
	double C = ImgSrc[idx2].x * cos((float(i) *M_PI / (sizeX * 2)));
	double S = ImgSrc[idx2].y * sin((float(i) *M_PI / (sizeX * 2)));
...
__global__ void devConstant(float* buf, int sizeX, int sizeY)
{
	...
	{		
		...
		if(i== 0 && j==0)
			...
			buf[idx] = buf[idx] / (2.0*cosf(4*M_PI*i / (4*sizeX - 1)) + 2.0*cosf(4*M_PI*j / (4*sizeY - 1)) - 4.0);
	}
}
//--------------------------------------------------------------------------------------
...
__global__ void NumericDerivative1(float *outX, float *outY, float *input, int sizeX, int sizeY)
{
	...
	/*if (i < sizeX && j < sizeY) {
		...
		{
			outX[idx] = (input[idx + 1] + input[idx - 1]) - 2 * input[idx];
			outY[idx] = (input[idx + sizeX] + input[idx - sizeX]) - 2 * input[idx];
		}
		else {
			outX[idx] = 0;
			outY[idx] = 0;
	...
	/*if (i<sizeX && j<sizeY)
	{
		outX[idx] = (i == sizeX - 1) ? input[idx] - input[idx - 1] : input[idx + 1] - input[idx];//- input[idx - 1    ]
		outY[idx] = (j == sizeY - 1) ? input[idx] - input[idx - sizeX] : input[idx + sizeX] - input[idx];//- input[idx - sizeX] 
	}*/
}
//--------------------------------------------------------------------------------------
...
__global__ void TrigonometricF(float *outX, float *outY, int sizeX, int sizeY)
{
	...
	unsigned int j = blockDim.y * blockIdx.y + threadIdx.y;
	unsigned int idx;
	float signOutX, signOutY;
	if (i < sizeX && j < sizeY)	{
		idx = i + sizeX*j;
		signOutX = signbit(outX[idx]) ? -1 : 1;
		signOutY = signbit(outY[idx]) ? -1 : 1;
		outX[idx] = outX[idx] - 2 * M_PI*signOutX * floor(fabs(M_PI * signOutX + outX[idx]) / (M_PI * 2));
		outY[idx] = outY[idx] - 2 * M_PI*signOutY * floor(fabs(outY[idx] + M_PI * signOutY) / (M_PI * 2));
	}
}
//--------------------------------------------------------------------------------------
